...
void Object::Initialize(const QOpenGLTexture::Filter minFilter,
	const QOpenGLTexture::Filter magnFilter, const bool anisotropy)
{
	if (!setMesh()) {
		return;
	}
	...
	TextureLoader::Load(std::string("Textures/" + _name + ".jpg"), _mesh.texture);
	_mesh.texture->setMinMagFilters(minFilter, magnFilter);
	_mesh.texture->setMaximumAnisotropy(!anisotropy ? 0 : GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT);

	TextureLoader::Load(std::string("Textures/" + _name + "_NormalMap.jpg"), _mesh.normalMap);
}
